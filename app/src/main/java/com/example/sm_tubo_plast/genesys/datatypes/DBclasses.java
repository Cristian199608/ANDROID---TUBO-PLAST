...
/*
	...
	public ArrayList<HashMap<String, Object>> getProgramacionxDia2( String txtBusqueda,
		...
		rawQuery = "select * from (" +
				"select distinct(nomcli) as nomcli,ruccli,fecha_compra,monto_compra," +
				"cliente.codcli,item_dircli,n_dia,ifnull(observacion,'') as observacion, " +
				"ifnull((" +
		...
		rawQuery="WITH consultaDeudas AS (\n" +
				"\n" +
				"SELECT\n" +
				"    codcli as codclix,\n" +
				"\n" +
				"    SUM(case " +
				"		when date('now', 'localtime') < date(fecha_vencimiento) then total else 0 end" +
				"    ) AS deuda_x_vencer,\n" +
				"\n" +
				"    SUM(\n" +
				"        CASE\n" +
				"            WHEN julianday('now') - julianday(date(fecha_vencimiento)) > 0\n" +
				"             AND julianday('now') - julianday(date(fecha_vencimiento)) < 30\n" +
				"            THEN saldo\n" +
				"            ELSE 0\n" +
				"        END\n" +
				"    ) AS deuda_menor_30_dias,\n" +
				"\n" +
				"    SUM(\n" +
				"        CASE\n" +
				"            WHEN julianday('now') - julianday(date(fecha_vencimiento)) >= 30\n" +
				"            THEN saldo\n" +
				"            ELSE 0\n" +
				"        END\n" +
				"    ) AS deuda_mayor_igual_30_dias\n" +
				"\n" +
				"FROM cta_ingresos\n" +
				"GROUP BY codcli \n" +
				"\n" +
		...
		rawQuery += "\nselect * from (" +
				...
				"monto_compra," +
				"cliente.codcli," +
				"dc.item as item_dircli, 1 as n_dia,ifnull(observacion,'') as observacion, " +
				...
				"limite_credito," +
				"disponible_credito," +
				"ifnull(deuda_menor_30_dias, 0) as deuda_menor_30_dias, " +
				"ifnull(deuda_mayor_igual_30_dias, 0) as deuda_mayor_igual_30_dias, " +
				"ifnull(deuda_x_vencer, 0) as deuda_x_vencer, " +
				"case when (ifnull(deuda_mayor_igual_30_dias, 0) + ifnull(deuda_menor_30_dias, 0))>0 " +
				"then 'Con deuda' else 'Sin deudas' end as estadoDeuda," +
				"ifnull((SELECT 'L' FROM lugarEntrega le " +
				"where le.codigoCliente= cliente.codcli " +
				"and cast(ifnull(le.latitud, '0') as numeric)!=0.0 limit 1 ), \n" +
				"'') as estado_localizacion  " +
				" from cliente " +
				"inner join (" +
				"select dcx.codcli, dcx.item from direccion_cliente dcx " +
				") dc on cliente.codcli=dc.codcli\n"
				+"left join consultaDeudas on consultaDeudas.codclix=cliente.codcli "
				+"where " +
				"cliente.codcli in (" +
				"select x.codcli from znf_programacion_clientes x " +
				"where x.codven= '"+codven+"' " +
				")\n " +
				"and (nomcli like '%"+txtBusqueda+"%' or ruccli like '%"+txtBusqueda+"%' or direccion like '%"+txtBusqueda+"%' ) "
				+ " limit "+start+", "+end+" "
				+") result "
		...
		while (cur.moveToNext()) {
			...
			dbc.put("item_direccion", cur.getString(5));
...
//			String estado_localizacion = getEstadoDireccionCliente(dbc.get("codigo").toString(), dbc.get("item_direccion").toString());
			...
			dbc.put("estado_localizacion", cur.getString(cur.getColumnIndex("estado_localizacion")));
			...
			dbc.put("limite_credito", cur.getDouble(cur.getColumnIndex("limite_credito")));
			dbc.put("disponible_credito", cur.getDouble(cur.getColumnIndex("disponible_credito")));
			dbc.put("deuda_menor_30_dias", cur.getDouble(cur.getColumnIndex("deuda_menor_30_dias")));
			dbc.put("deuda_mayor_igual_30_dias", cur.getDouble(cur.getColumnIndex("deuda_mayor_igual_30_dias")));
			dbc.put("estadoDeuda", cur.getString(cur.getColumnIndex("estadoDeuda")));
			dbc.put("deuda_x_vencer", cur.getDouble(cur.getColumnIndex("deuda_x_vencer")));
			dbcliente.add(dbc);
		}
		cur.close();
	...
	public ItemProducto[] getProductosXTIME_SYNC(String timeSincronizacion) {
			...
			rawQuery = "select * from "
					...
					+ "producto.factor_conversion,"
					+ "producto.peso,"
					+ "ifnull(mta_kardex.stock,0) as stock,"
					+ "producto.cod_rapido, "
					+ "producto.afecto, "
					+ "producto.estado,"
					+ "producto.desc_comercial,  "
			...
			if (cursor.moveToFirst()) {
				...
				do {
					...
					productos[i].setEstado(cursor.getString(11));
					productos[i].setDesc_comercial(cursor.getString(cursor.getColumnIndex("desc_comercial")));
					productos[i].setCodunimed(cursor.getString(cursor.getColumnIndex("codunimed_almacen")));
	...
	public boolean AgregarPedidoDetallePromocion(DBPedido_Detalle item) {
		return AgregarPedidoDetallePrincipal(item, item.getItem());
	}
	...
	public ArrayList<Integer> getOrdenItemPedido(String oc_numero){
		ArrayList<Integer> listaIndex=new  ArrayList<Integer>();
		SQLiteDatabase db = getReadableDatabase();
		String rawQuery = "SELECT \n" +
				"pd.item, ifnull(rb.salida_item , 0) as itemSalida \n" +
		...
		Cursor cur = db.rawQuery(rawQuery, null);
		while (cur.moveToNext()) {
			int itemIndex1 = cur.getInt(0);
			int itemIndex2 = cur.getInt(1);
			if(!listaIndex.contains(itemIndex1) && itemIndex1>0)
				listaIndex.add(listaIndex.size(), itemIndex1);
			if(!listaIndex.contains(itemIndex2) && itemIndex2>0)
				listaIndex.add(listaIndex.size(),itemIndex2);
		}
		cur.close();
		db.close();
		return listaIndex;
	...
	public int cantidadProductosSinStock(String oc_numero){
		String sqlw =  "select count(*) from pedido_detalle " +
				"where oc_numero='" + oc_numero + "'" +
				"and flagStockValido=0 " ;
		SQLiteDatabase db= getReadableDatabase();
		Cursor cursor=db.rawQuery(sqlw, null);
		int cantidadi=cursor.moveToNext()?cursor.getInt(0):0;
		cursor.close();
		db.close();
		return cantidadi;
	...
	public ItemProducto[] obtenerListadoProductos_pedido(String oc_numero) {
		...
		ArrayList<Integer> listaIndex = this.getOrdenItemPedido(oc_numero);
		String whereAdd;
		...
		//-------------------------------ORDENAMIENTO DEL PEDIDO----------------------------------------------------------------
		ItemProducto[] productosOrdenados=new ItemProducto[productos.length];
		for (int i = 0; i < listaIndex.size(); i++) {
			for (int j = 0; j < productos.length; j++) {
				if(productos[j].getItem()==listaIndex.get(i)){
					productosOrdenados[i]=productos[j];
					break;
				}
			}
		}
	...
	public ItemProducto[] obtenerListadoProductos_pedidoMAIN(String whereAdd) {
		...
		String sqlRAW =
				"select "
						...
						+ "pedido_detalle.flagStockValido "
						...
						+ "where 5 = 5 "+whereAdd+" "
						+ " order by cast(pedido_detalle.item  as INTEGER) asc";
		...
		if (cursor.moveToFirst()) {
			...
			do {
				...
				productos[i].setFlagStockValido(cursor.getInt(cursor.getColumnIndex("flagStockValido")));
	...
	public boolean modificarCantidadItemPedido(int nCantidad,
		...
		try {
			...
			DBPedido_Detalle dnPedidoItem = getPedidosDetalleEntityWithItem(oc_numero, codprod, nroItem);
			SQLiteDatabase db = getWritableDatabase();
			...
			double precioListaTotal= VARIABLES.getDoubleFormaterFourDecimal(precioLista*nCantidad);
			double precioUnitTotal= VARIABLES.getDoubleFormaterFourDecimal(precioUnit*nCantidad);

			double montoDscUnit  = VARIABLES.getDoubleFormaterThreeDecimal(precioListaTotal- precioUnitTotal);
			double pesoTotal= VARIABLES.getDoubleFormaterFourDecimal(peso*nCantidad);

			ContentValues reg = new ContentValues();
			reg.put("cantidad", nCantidad);
			reg.put("precio_neto", precioUnitTotal);
			reg.put("peso_bruto", pesoTotal);
			reg.put("descuento", montoDscUnit);
			db.update("pedido_detalle", reg, where, args);
		...
		try {
			for (int i = 0; i < jArray.length(); i++) {
				...
				if(!verificarPedidoTieneCabecera(db, jsonData.getString("oc_numero").trim()) || jsonData.getString("oc_numero").contains("TPLAST") ){
					...
					cv.put(Pedido_cabecera.flg_aprobacion, 			jsonData.getString(Pedido_cabecera.flg_aprobacion).trim());
					cv.put(Pedido_cabecera.numdoc, 			jsonData.getString(Pedido_cabecera.numdoc).trim());
...
//							if(ax<=0) throw new SQLiteException("Error no se pudo registrar");
						...
						final Type mallax = new TypeToken<ArrayList<Pedido_detalle2>>() {}.getType();
						final ArrayList<Pedido_detalle2> listaDet2 = gson.fromJson(jsonData.getString("pedidoDetalle2"), mallax);
						for (Pedido_detalle2 pedido_detalle2 : listaDet2) {
							daoPedidoDet2.InsertItem(pedido_detalle2, db);
						}
						...
						for (int j = 0; j < json_registroBonificaciones.length(); j++) {
							...
							cv2.put(DBtables.TB_registro_bonificaciones.ENTRADA_ITEM, jsonData_reg.getInt(DBtables.TB_registro_bonificaciones.ENTRADA_ITEM));
							cv2.put(DBtables.TB_registro_bonificaciones.SALIDA_ITEM, jsonData_reg.getInt(DBtables.TB_registro_bonificaciones.SALIDA_ITEM));
...
//					}
	...
	public DBPedido_Detalle getPedidosDetalleEntityWithItem(String oc, String codpro, int item) {
		String rawQuery;
...
Log.e("getPedidosDetalleEntity","Oc_numero: "+cur.getString(0));
	...
	public ArrayList<DB_ObjPedido> getTodosObjPedido_json_flagp() {
		String addWhere = " AND flag IN ('P','I') " +
				" AND (flg_aprobacion = 0 " +
				" or NOT EXISTS (" +// o si es que no hay pendientes por aprobacion o que no hay desaprobados
				"     SELECT 1 FROM workflow_pedido wp " +
				"     WHERE wp.estado IN (0, -1) " +
				"		and wp.oc_numero = pedido_cabecera.oc_numero " +
				" )" +
				") " +
				"and NOT EXISTS(" +
				"		select 1 from pedido_detalle pd " +
				"		where pd.flagStockValido=0 " +
				"		and pd.oc_numero = pedido_cabecera.oc_numero " +
				")";
	...
	public DBPedido_Cabecera getPedido_cabecera(String oc_numero) {
		...
		rawQuery = "select * from pedido_cabecera " +
				"where oc_numero='"+ oc_numero + "'" +
				"or '"+oc_numero+"' like 'TPLAST%' ";
	...
	public ArrayList<DB_PromocionDetalle> getPromocionesXProducto2(	String codpro, int tipo_unimed_entrada, String codcli,	String codven, String cod_politica, String codigoUbigeo) {	
		...
		rawQuery = "select * from promocion_detalle " +
				"where entrada like '"+ codpro+ "' and tipo_unimed_entrada like '"+ tipo_unimed_entrada+ "' "
	...
	public void EliminarPedidoDetalle2Enviados(String codven) {

		String where = "oc_numero in (select oc_numero from pedido_cabecera where flag not in (?) or cod_emp <> ?)";
		String[] args = { "P",  codven };

		try {
			SQLiteDatabase db = getWritableDatabase();

			long pr = db.delete(""+DBtables.Pedido_detalle2.TAG, where, args);
			db.close();
			Log.i(TAG,
					"EliminarMovimiento_imagenes_enviados:: eliminados los "+pr+" Movimiento_imagen de las cabeceras con estado <> 'P'");
	...
	public int sincronizar_lugarEntregaGeolocalizacion(JSONArray jArray, String fecha, int start) throws JSONException {
		...
		try {
			...
			for (int i = 0; i < jArray.length(); i++) {
				jsonData = jArray.getJSONObject(i);
				argx=new String[]{jsonData.getString(DBtables.LugarEntrega.CODIGO_CLIENTE).trim(),
						jsonData.getString(DBtables.LugarEntrega.ITEM_SUCURSAL).trim(),
						jsonData.getString(DBtables.LugarEntrega.CODIGO_LUGAR).trim()};

				cv.put(DBtables.LugarEntrega.latitud,jsonData.getDouble(DBtables.LugarEntrega.latitud));
				cv.put(DBtables.LugarEntrega.longitud,jsonData.getDouble(DBtables.LugarEntrega.longitud));
				db.update(DBtables.LugarEntrega.TAG, cv, where,argx);
				Log.i(TAG+":sincronizar_lugarEntrega:", i+" "+jsonData.getString(DBtables.LugarEntrega.CODIGO_LUGAR).trim()+"  "+jsonData.getString(DBtables.LugarEntrega.DIRECCION_ENTREGA).trim());
			}
			db.setTransactionSuccessful();
	...
	public boolean AgregarPedidoCabecera_raiz(DB_ObjPedido cabecera, SQLiteDatabase DB) {
		...
		try {
			...
			Nreg.put(Pedido_cabecera.FLAG_DESPACHO, cabecera.getFlagDespacho());
	...
	public String obtenerEstadoGelocalizacioCliente (String codcli ) {
		String rawQuery = "select ifnull(estado,'') from " +
				"lugarEntrega " +
				"where codigoCliente='" + codcli + "' " +
				//"and itemSucursal = '"+sucursal+"' " +
				"and cast(ifnull(latitud, '0') as numeric)!=0.0 " +
				"limit 1";
		SQLiteDatabase db = getReadableDatabase();
		Cursor cur = db.rawQuery(rawQuery, null);
		String estado = "";
	...
	public void updateGeolocalizacionClienteFlag(String codigoCliente, String codigoSucursal) {
	...
	public String getEstadoDireccionCliente(String codigoCliente, String item) {
		String rawQuery;
		rawQuery = "SELECT ifnull(estado,'') FROM "+DBtables.Direccion_cliente.TAG+" " +
				"WHERE codcli like '"+codigoCliente+"' and item like '"+item+"'";
	...
	public ArrayList<DB_DireccionClientes> getLocalizacionesCliente(String codcli, String item){ //**Nuevo Localizacion
		...
		rawQuery= "select le.codigoCliente as codcli, " +
				"le.itemSucursal as item, ifnull(le.latitud,''), ifnull(le.longitud,''), le.estado, -1 as altitud," +
				"le.codigoLugar "
				+ "from lugarEntrega le " +
				"where  le.codigoCliente ='"+codcli+"' " +
				"and le.itemSucursal ='"+item+"'";
		...
		while (!cur.isAfterLast()) {
			...
			DB_DireccionClientes dbdireccion = new DB_DireccionClientes();
			dbdireccion.setCodcli(cur.getString(0));
			dbdireccion.setItem(cur.getString(1)+"::"+cur.getString(cur.getColumnIndex("codigoLugar")));
	...
	public void setDataPruebas() {
		...
		String sql = "update " + DBtables.Promocion_Detalle.TAG + " set  " +
				...
				//"agrupado=1, " +
				//"total_agrupado=1 " +
				 "where secuencia in ('11', '7') ";
		//16
//		sql = "UPDATE PEDIDO_CABECERA set flag='P' ";
//		String sql="update producto set despro='COMBOX '||despro where codpro='ISO18167X1M' ";
		//String sql="update promocion_detalle set tipo_promocion='XCOMBO' where secuencia='11'";
		db.execSQL(sql);
		db.close();

	}

	public void updatePedidoDetalle_SecuenciaPromocion(String oc_numero,String sec_promo, int item_promo, int prioridad, int salidaItem) {


		DAO_RegistroBonificaciones dao_reg=new DAO_RegistroBonificaciones(_context);
		ArrayList<DB_RegistroBonificaciones> listaBonificados=dao_reg.ObtenerRegistroBonificacionesBY_sec_promo(oc_numero, Integer.parseInt(sec_promo), salidaItem);

		String inCodpro="'vacio'";
		for (DB_RegistroBonificaciones boni:listaBonificados){
			inCodpro=inCodpro+", '"+boni.getEntrada()+"::"+boni.getEntrada_item()+"'";
		}


		String where = "oc_numero = ? and cip||'::'||cast(item as TEXT) in("+inCodpro+") and tipo_producto='V'";
		String[] args = { oc_numero};
		...
		try {
			...
			ContentValues reg = new ContentValues();
			if (prioridad==0){
				reg.put("sec_promo", sec_promo);
				reg.put("item_promo", item_promo);
			}else if (prioridad==1){
				reg.put("sec_promo_prioridad", sec_promo);
				reg.put("item_promo_prioridad", item_promo);
			}

			long i=db.update(""+DBtables.Pedido_detalle.TAG, reg, where, args);
			db.close();

			Log.i("udatePedidoDetalle_SecuenciaPromocion:: ", "SecuenciaPromocion UPDATE a '"
					+ inCodpro + "' es "+(i>=1));
...
//				-------------------------------ahora insertamos precios----------------------------------------------------------------+
	...
	public String guardarSyncClientesMasivo(ArrayList<ResultClienteCantol> listaClientes, String codven) {
		...
		try {
			...
			for (ResultClienteCantol cliente : listaClientes) {
				...
				values.put("comprobante",
						(cliente.getTipoDocumento()!=null && cliente.getTipoDocumento().equals("RUC"))
...
//				values.put("valido", cliente.getValido());
				...
				values.put("codven", codven);
				values.put("n_dia", 1);
				values.put("sec_zona", 1);
				values.put("codcli", cliente.getCodigoCliente());
				values.put("item_dircli", 0);
				values.put("sec_ruta", 0);
				long ax = db.insertWithOnConflict(DBtables.ZnfProgramacionClientes.TAG, null,values, SQLiteDatabase.CONFLICT_IGNORE);
			}
			db.setTransactionSuccessful();
			return null;
		} catch (Exception e) {
	...
	public String guardarSyncLugarEntregaClientesMasivo(ArrayList<ResultClienteLugarEntrega> listaLugarEntrega) {
		...
		try {
			String where = "estado != ? ";
			String[] args = { "P" };
			long dd = db.delete(DBtables.Direccion_cliente.TAG, where, args);
			long d = db.delete(DBtables.LugarEntrega.TAG, where, args);
			Log.i(TAG, "CANT ELIMINADO LUGAR ENTREGA "+d);
			String codigoDirFiscal="DOMICILIO_FISCAL";
			ContentValues values=null;
			for (ResultClienteLugarEntrega cliente : listaLugarEntrega) {

				values = new ContentValues();
				if(cliente.getCodigo_punto_entrega().toUpperCase().equals("DOMICILIO FISCAL")
				|| cliente.getCodigo_punto_entrega().toUpperCase().equals("DOMICILIO_FISCAL")
					...
					values.put(DBtables.Direccion_cliente.PK_ITEM,codigoDirFiscal);
					...
					values.put(DBtables.Direccion_cliente.CODDEP,cliente.getDepartamento());
					values.put(DBtables.Direccion_cliente.UBIGEO,cliente.getDistrito());
					values.put(DBtables.Direccion_cliente.DES_CORTA, cliente.getDireccion());

					values.put(Direccion_cliente.LATITUD, cliente.getLatitud().trim().isEmpty()?"0.0":cliente.getLatitud());
					values.put(Direccion_cliente.LONGITUD, cliente.getLongitud().trim().isEmpty()?"0.0":cliente.getLongitud());
					values.put(DBtables.Direccion_cliente.DOC_ADICIONAL,"");
					values.put(DBtables.Direccion_cliente.ESTADO,"P");
					values.put(Direccion_cliente.altitud,"0");
					long az = db.insertWithOnConflict(DBtables.Direccion_cliente.TAG, null, values,SQLiteDatabase.CONFLICT_IGNORE);
				}
				else {
					values = new ContentValues();
					values.put("codigoCliente", cliente.getCodigo_cliente());
					values.put("itemSucursal", codigoDirFiscal);
...
//					values.put("zona", cliente.getZona());
			...
			//-------------------------------actualizamos direccion fiscal----------------------------------------------------------------
			db.execSQL("UPDATE cliente\n" +
					"SET direccionFiscal = ifnull((\n" +
					"    SELECT dc.direccion\n" +
					"    FROM direccion_cliente dc\n" +
					"    WHERE dc.codcli = cliente.codcli \n" +
					"\tlimit 1\n" +
					"), 'No se encontró dirección fiscal')");
	...
	public String guardarSyncFormaPagoMasivo(ArrayList<ResultCondicionVenta> listaCondicionVenta, boolean deleteAll) {
	...
	public void registrarDatosUsuarioLogin(String codven, String nombreVendedor){
		...
		cv.put(DBtables.Usuarios.USEPAS, "");
		cv.put(DBtables.Usuarios.USENAM, nombreVendedor);
		cv.put(DBtables.Usuarios.USEUSR, "");
		cv.put(DBtables.Usuarios.USESGL, "");
		cv.put(DBtables.Usuarios.codigoRol,"");
		db.insertWithOnConflict(DBtables.Usuarios.TAG, null, cv, SQLiteDatabase.CONFLICT_REPLACE);

		cv = new ContentValues();
		cv.put(DBtables.Vendedor.CODVEN, codven);
	...
	public void guardarWorkFlowPedido(String ocNumero, ArrayList<WorkflowPedido> workflowPedido) {
		...
		String where="oc_numero = ?";
		String[] argCab = new String[]{ocNumero};
		cv = new ContentValues();
		cv.put("flg_aprobacion", 1);
		db.update(DBtables.Pedido_cabecera.TAG, cv, where, argCab);
		//-------------------------------limpiando----------------------------------------------------------------
		where="oc_numero = ? and estado<=-1";
	...
	public void registrarPedidoDetalleDescuento(ArrayList<PedidoDetalleDescuento> lista ){
		...
		try {
			...
			for (PedidoDetalleDescuento obj : lista) {
				if(!isEliminado){
					String where = "oc_numero=? and item=?";
					String[] args = { obj.getOc_numero(), ""+obj.getItem() };
					db.delete(DBtables.Pedido_detalle_descuento.TAG, where, args);
					isEliminado=true;
				}
				if(obj.getPcjt_desc()<=0.0) continue;
				ContentValues values = new ContentValues();
	...
	public Cursor GetCondicionDePromocionx(String codven, String codcli,  int secuencia_promocion,  String codpro) {
		String rawQuery;
		rawQuery = "select pd.secuencia," +
				"pd.item, " +
				"pd.promocion, " +
				"pd.entrada, " +
				"pd.salida, " +
				"case when 0 /*pd.desde+pd.hasta*/>0 \n" +
				"then 'Por Rango'\n" +//acumulable
				"when   cast('0' /*pd.escalable*/  as int)=1 \n" +
				"then 'Por Escalable' \n" +
				"else 'Simple' end as rango_o_escalable,\n" +
				"case when  cast(pd.acumulado  as int)=1 then 'ACUMULADO' else 'No Acumulable' end as acumulable,\n" +
				"pd.total_agrupado,  "+ // 1= or , 2>=and
				"case when  cast(pd.total_agrupado  as int)>1 then 'AGRUPADO' else ' ' end as promocion_and_or,  "+
				"pd.tipo_promocion, " +
				"pd.condicion,  " +
				"pd.tipo, "+
				"pd.monto,"+
				"pd.cant_condicion,"+
				"case pd.tipo_promocion  when 'XDESCUENTO' then 'descuento' when 'XDSCMONEDA' then 'desc moneda' else  '' end as  tipo_promocion_txt, "+
				"(select um.desunimed from unidad_medida um where um.codunimed= case when pd.tipo_unimed_entrada=0 then pEntrada.codunimed_almacen else pEntrada.codunimed end ) as unimedE,\n" +
				"case pd.tipo_promocion  when 'XDESCUENTO' then '%' when 'XDSCMONEDA' then ' en S/' else ifnull((select um.desunimed from unidad_medida um where um.codunimed= p.codunimed_almacen), '')  end as unimedS,\n" +
				"pd.cant_promocion,\n" +
				"p.despro, \n" +
				"pEntrada.despro as producto_entrada, \n" +
				"pd.general as clienteLimitado, \n" +
				"pd.vendedor as vendedorLimitado, \n" +
				"pd.politica as politicaLimitado, \n" +
				"0 as desde , \n" + //pd.desde
				"0 as hasta, \n" +//pd.hasta
				...
				"inner join producto pEntrada on pEntrada.codpro=pd.entrada\n" +
				"where pd.entrada like '"+codpro + "' " +
				"and pd.secuencia = case when "+secuencia_promocion+" > 0 then '"+secuencia_promocion +"' else pd.secuencia end "+
				"and (pd.secuencia in ( select pv.sec_promocion from promocion_vendedor pv where pv.codven='"+codven+"' )" +
				"     or pd.vendedor =0 " +
				"   ) " +
				"and (pd.secuencia in (select pc.sec_promocion from promocion_clientes pc where pc.codcli like '"+codcli+"')" +
				"     or pd.general =0 " +
				"   ) " +
				"order by pd.secuencia asc ";

		Log.i(TAG, "GetCondicionDePromocionx sql "+rawQuery);
		SQLiteDatabase db = getReadableDatabase();
		Cursor cur = db.rawQuery(rawQuery, null);
